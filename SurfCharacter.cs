...
namespace Fragsurf.Movement {
    ...
    public class SurfCharacter : MonoBehaviour, ISurfControllable {
        ...
        private void Awake () {
            ...
            // Disable collisions with layer 6 for this GameObject and its collider
            SetupLayerCollisionIgnoring();

        }

        private void SetupLayerCollisionIgnoring() {
            // Get the current layer of this GameObject
            int currentLayer = gameObject.layer;
            
            // Ignore collisions between current layer and layer 6
            Physics.IgnoreLayerCollision(currentLayer, 6, true);
            
            // Also set up specific collider ignoring in Start() for additional safety
        }
        ...
        private void Start () {
            ...
            // After creating the collider, ignore all existing layer 6 colliders
            IgnoreLayer6Colliders();
        ...
        private void IgnoreLayer6Colliders() {
            // Find all colliders on layer 6 and ignore them specifically
            Collider[] layer6Colliders = FindObjectsOfType<Collider>();
            foreach (Collider col in layer6Colliders) {
                if (col.gameObject.layer == 6 && col != _collider) {
        ...
        private void Update () {
            ...
            // Triggers - Filter out layer 6 objects
            if (numberOfTriggers != triggers.Count) {
                numberOfTriggers = triggers.Count;

                underwater = false;
                triggers.RemoveAll (item => item == null || item.gameObject.layer == 6);
                foreach (Collider trigger in triggers) {

                    if (trigger == null || trigger.gameObject.layer == 6)
                        continue;
        ...
        private void OnCollisionStay (Collision collision) {
            ...
            // Ignore collisions with layer 6 objects
